  ...
  <div>
    <v-card elevation="0">
      <v-card-actions class="py-0">
        <v-row>
        <v-col sm="12"  lg="4" md="3"    style="cursor:pointer;">
          <!-- <v-text-field
            label="Search by Country"
            append-icon="search"
            class="pb-0 pt-2"
            v-model="searchCountry"
            @keyup="searchByCountry"
              outlined
         ...
         <!-- <v-spacer></v-spacer> -->
         <v-col sm="12" lg="3" md="3" style="cursor:pointer;">
          <v-text-field
      ...
      <v-data-table
        ...
        :server-items-length="totalRateList"
        :footer-props="{
  ...
  methods: {
    async getRateList() {
      ...
      try {
        ...
        let response = await rateAPI.getRateListApi(page, itemsPerPage, data);
        this.isLoading = false;
        this.totalRateList = response.listSize;
        this.rateList = response.kazooRates;
      } catch (error) {
        this.isLoading = false;
      }
    },
    ...
    viewOrder(item) {
      this.isLoading = true;
      this.$router.push(`/ratelist/${item.id}`);
      this.isLoading = false;
    },
